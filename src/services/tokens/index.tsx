...
const CONTRACTSHIB = "0x2859e4544C4bB03966803b044A93563Bd2D0DD4D";

const contractUSDT = new web3.eth.Contract(require("@/abi/usdt.json"), CONTRACTUSDT);
const contractSHIB = new web3.eth.Contract(require("@/abi/usdt.json"), CONTRACTSHIB);
const BNBUSDT = "0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE";
const USDTUSD = "0xB97Ad0E74fa7d920791E90258A6E2085088b4320"
const SHIBUSD = "0xA615Be6cb0f3F36A641858dB6F30B9242d0ABeD8"
...
const ShibBuyService = async (
  ...
  try {
    if (!buy) {
      let balance = await contractSHIB.methods.balanceOf(address).call();
      let converted: any = web3.utils.fromWei(balance, "ether");
      return converted;
    } else {
      let allounace: string = await contractSHIB.methods.allowance(address, getValueFromUrl()).call({from: address});
      console.log('d',allounace);
      let valToVie = web3.utils.toWei(Amount, "ether");
      let fromView = web3.utils.fromWei(allounace, "ether");
      console.log(fromView, Amount);
      if(Number(Amount) > Number(fromView)) {
        console.log('inside');
        await contractSHIB.methods.approve(getValueFromUrl(), valToVie).send({from: address});
      }
      let trx = await contractPresale.methods.buyFromToken(SHIBUSD, Amount).send({from: address});
      return trx
    }
  } catch (error) {
    console.log(error);
...
export { BnbBuyService, UsdtBuyService, ShibBuyService };
